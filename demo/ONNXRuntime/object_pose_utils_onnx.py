...
#                              [ 0.0, 0.0, 1.0]], dtype=np.float32)
lm_camera_matrix = np.array([[601.949, 0.0, 327.582],
                             [0.0, 601.940, 247.230],
                             [ 0.0, 0.0, 1.0]], dtype=np.float32)   #####~ 修改相机内参

# lm_camera_matrix = np.array([[858.64, 0.0, 662.87],
#                              [0.0, 883.45, 362.25],
#                              [ 0.0, 0.0, 1.0]], dtype=np.float32)
...
def project_3d_2d(pts_3d, rotation_mat, translation_vec, camera_matrix, cls):
...
def draw_obj_pose(origin_img, dets, class_names, class_to_cuboid, camera_matrix, conf_thres=0.85):   
    ...
    for det in dets:
        ...
        if score>conf_thres:
            ...
            rotation_mat = np.concatenate((r1, r2, r3), axis=1) ## 旋转矩阵
            ...
            translation_vec[1] = Y
            # if cls == 0:   #####~
            cuboid_corners_2d = project_3d_2d(class_to_cuboid[int(cls)], rotation_mat, translation_vec, camera_matrix, cls)
