...
_SUPPORTED_DATASETS = ["coco", "lm", "lmo", "ycbv", "tless", "coco_kpts","my"]
_NUM_CLASSES = {"coco":80, "lm":15, "lmo": 8, "ycbv": 21, "tless": 30, "coco_kpts":1,"my":1}
_VAL_ANN = {
    "coco":"instances_val2017.json", 
    "lm":"instances_test.json",
    "lmo":"instances_test_bop.json",
    "my":"instances_test_bop.json",   #####~
...
if __name__ == "__main__":
    ...
    available_gpus = get_num_devices()
    num_gpu = min(available_gpus,args.devices) if args.devices is None else available_gpus
    # num_gpu = get_num_devices() if args.devices is None else args.devices
    assert num_gpu <= get_num_devices()
