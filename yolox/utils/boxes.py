...
def postprocess(prediction, num_classes, conf_thre=0.7, nms_thre=0.45, class_agnostic=False, human_pose=False):
    ...
    output = [None for _ in range(prediction.shape[0])]   ##### [None for _ in range(len(prediction))]
    for i, image_pred in enumerate(prediction.unbind(0)):   ##### enumerate(prediction):

        # If none are remaining => process next image
        if image_pred.size(0) == 0:   ##### if not image_pred.size(0):
            continue
